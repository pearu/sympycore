...
class Base(object):
    ...
    def handler(self):
        if self._handler is None:
            raise TypeError('no handler registered, see register_handler method.')
        return self._handler

    _handler = None
    @staticmethod
    def register_handler(handler):
        """Register handler. Returns previous handler object.

        handler can be any object having the following callable attributes:
          Symbol
          Integer
          Float
          Lambda
          predefined_objects - dictionary used to return predefined identifiers
          Range, Keyword
        """
        old_handler = Base._handler
...
class Lambda_Test(Base):
    ...
    def tosymbolic(self, commutative=True):
        ...
        return self.handler.Lambda(expr, *ids)
...
class Identifier(StringBase):
    ...
    def tosymbolic(self, commutative=True):
        obj = self.handler.predefined_objects.get(self.string)
        if obj is not None:
            return obj
        #XXX: should we handle dummy symbols here?
        return self.handler.Symbol(self.string)
...
class Int_Literal(NumberBase):
    ...
    def tosymbolic(self, commutative=True):
        return self.handler.Integer(int(self.items[0]))
...
class Float_Literal(NumberBase):
    ...
    def tosymbolic(self, commutative=True):
        return self.handler.Float(self.items[0])
...
class Logical_Literal(NumberBase):
    ...
    def tosymbolic(self, commutative=True):
        return self.handler.Symbol(self.items[0].upper())
...
class Argument(KeywordValueBase):
    ...
    def tosymbolic(self, commutative=True):
        ...
        if isinstance(e,(list, tuple)):
            return self.handler.Range(s, *e)
        return self.handler.Keyword(s, e)
